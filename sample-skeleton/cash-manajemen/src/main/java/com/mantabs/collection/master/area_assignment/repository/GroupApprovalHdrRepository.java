...
public interface GroupApprovalHdrRepository extends  JpaRepository<GroupApprovalHdr, String> {
	...
	@Query(value = " update mmf_master.mmf_mst_group_approval_Hdr "
			+ " set status = 0 "
			+ " where jenis_biaya =:jenisBiaya ", nativeQuery = true)
	...
	@Query(" select a from GroupApprovalHdr a "
			+ " where a.kategoriBiaya =:kategoriBiaya ")
	public List<GroupApprovalHdr> listGroupApprovalHdrWithKategoriBiaya (
			@Param("kategoriBiaya")String kategoriBiaya
			);
	
	@Query(" select a from GroupApprovalHdr a "
			+ " where a.subKategori =:subKategori ")
	public List<GroupApprovalHdr> listGroupApprovalHdrWithSubKategori (
			@Param("subKategori")String subKategori
			);
	
	@Query(" select a from GroupApprovalHdr a "
			+ " where a.jenisBiaya =:jenisBiaya ")
	public List<GroupApprovalHdr> listGroupApprovalHdrWithJenisBiaya (
			@Param("jenisBiaya")String jenisBiaya
			);
